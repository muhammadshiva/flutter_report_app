...
class AyakManualQueryController extends GetxController {
  ...
  void validateForm() {
    ...
    print('VALUE TEST: ${tanggalTxt.value}');
    print('VALUE TEST: ${sumberBatokTxt.value}');
    print('VALUE TEST: ${batokTxt.value}');
    print('VALUE TEST: ${batokMentahTxt.value}');
    print('VALUE TEST: ${granulTxt.value}');
    print('VALUE TEST: ${tanggalTxt.value}');
  ...
  Future<void> postAyakManual() async {
    try {
      ...
      if (GlobalController.to.isConnect.isTrue) {
        ...
        AyakManualPostResponseModel response = await AyakManualRepository.postAyakManual(
        ...
        if (response.status == 200) {
          ...
          if (Get.isRegistered<AyakManualController>()) {
            var timer = Timer(const Duration(seconds: 3), () {
              Get.until((route) => Get.currentRoute == AppRoute.ayakManualRoute);
  ...
  void initEdit() {
    ...
    batokMentahTxt.value = argument.listAyakManual?.jumlahBatokMentah.toString() ?? '';
  ...
  //   isEdit.value =
  //   tanggalTxt.value =
  //   tanggalTxtInit.value =
  //   sumberBatokTxt.value =
  //   batokTxt.value =
